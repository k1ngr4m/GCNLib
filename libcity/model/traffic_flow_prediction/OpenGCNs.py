...
class TemporalAttentionLayer(nn.Module):
    ...
    def __init__(self, device, in_channels, num_of_vertices, num_of_timesteps):
        ...
        # 初始化参数
        self.U1 = nn.Parameter(torch.FloatTensor(num_of_timesteps, num_of_vertices).to(device))  # 时间注意力参数U1
        self.U2 = nn.Parameter(torch.FloatTensor(num_of_vertices, in_channels).to(device))  # 时间注意力参数U2
    ...
    def forward(self, x):
        ...
        lhs = torch.matmul(x.permute(0, 3, 2, 1), self.U1)  # (B, T, N)
        lhs = torch.matmul(lhs, self.U2)  # (B, T, in_channels)
        rhs = torch.matmul(self.U3, x)  # (B, in_channels, T)
        product = torch.matmul(lhs, rhs.transpose(-1, -2))  # (B, T, T)
        e = torch.matmul(self.Ve, torch.sigmoid(product + self.be))  # 应用sigmoid激活函数并计算最终的时间注意力分数
        e_normalized = F.softmax(e, dim=1)  # 对时间注意力分数进行归一化
        return e_normalized  # 返回时间注意力分数
...
class OpenGCNs(AbstractTrafficStateModel):
